...
const TransferGFTModal = ({ isOpen, setIsOpen, selectedGFT, refresh, setRefresh }) => {
    ...
    const handleSubmit = () => {
      ...
      if (!validator.isEmpty(formdata.email) && !validator.isEmail(formdata.email)) {
        ...
        contract.methods.safeTransferFrom(userAddress, formdata.address, selectedGFT.tokenId).send({from: userAddress})
        .then(res => {
            if (res.status) {
                axios.post("/api/gftshoppe/transfer/transfertoonlywallet", {sender: userAddress, formdata, tokenId: selectedGFT.tokenId, transactionHash: res.transactionHash})
                .then(res1 => {
                    if (res1.data.status) {
            ...
            <div className="relative w-full my-6 mx-auto max-w-lg">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none p-12">
                ...
                <div className="relative flex-auto">
                    ...
                    <div className="mb-4">
                        <input type="text" name="address" value={formdata.address} className={`shadow appearance-none border rounded-lg w-full py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${error.errorAddress ? 'border-red-500' : ''}`} placeholder="Receiver Wallet Address" onChange={handleInput} />
                        {error.errorAddress ? <p className="text-red-500 text-sm pt-1">Please indicate the correct receiver wallet address</p> : ''}
                    </div>
                    <div className="mb-4">
                        <input type="text" name="email" value={formdata.email} className={`shadow appearance-none border rounded-lg w-full py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${error.errorEmail ? 'border-red-500' : ''}`} placeholder="Receiver Email Address (optional)" onChange={handleInput} />
                        {error.errorEmail ? <p className="text-red-500 text-sm pt-1">Please indicate the correct receiver email address</p> : ''}
                    </div>
                    <div className="mb-4">
                        ...
                        {error.errorSenderEmail ? <p className="text-red-500 text-sm pt-1">Please indicate the sender email address</p> : ''}
                    </div>
                    <div className="mb-4">
